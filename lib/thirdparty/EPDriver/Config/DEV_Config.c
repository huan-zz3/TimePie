...
void DEV_Digital_Write(UWORD Pin, UBYTE Value)
{
    ...
    if (err)
        Debug("lgGpioWrite Failed, Pin = %d, Value = %d, err = %d\n", Pin, Value, err);
    else
        ;
    // Debug("lgGpioWrite Successed, Pin = %d, Value = %d\n", Pin, Value);
#elif USE_GPIOD_LIB
...
UBYTE DEV_Digital_Read(UWORD Pin)
{
    ...
    if (Read_value < 0)
        Debug("lgGpioRead Failed, Pin = %d, Read_value = %d\n", Pin, Read_value);
#elif USE_GPIOD_LIB
...
void DEV_GPIO_Mode(UWORD Pin, UWORD Mode)
{
    ...
    if (Mode == 0 || Mode == LG_SET_INPUT)
    {
        ...
        Debug("IN Pin = %d, err = %d\r\n", Pin, err);
    }
    else
    {
        int err = lgGpioClaimOutput(GPIO_Handle, LFLAGS, Pin, LG_LOW); // old GPIO_Handle, LFLAGS, Pin, LG_LOW
        Debug("OUT Pin = %d, err = %d\r\n", Pin, err);
    }
#elif USE_GPIOD_LIB
...
{
    Debug("DEV_GPIO_Init NOW!\n");
    EPD_RST_PIN = 4;  // old 17
    EPD_DC_PIN = 8;   // old 25
    EPD_CS_PIN = 9;   // old 8
    EPD_BUSY_PIN = 6; // old 24
    ...
    Debug("DEV_GPIO_Init - DEV_Digital_Write NOW!\n");
...
UBYTE DEV_ModuleInit(void)
{
    ...
    if (fp == NULL)
    {
        Debug("It is not possible to determine the model of the Raspberry PI (1)\n");
        return -1;
    }

    if (fgets(buffer, sizeof(buffer), fp) != NULL)
    {
        Debug("It is a Raspberry PI 5\n");
    ...
    {
        Debug("It is not possible to determine the model of the Raspberry PI (2)\n");
        GPIO_Handle = lgGpiochipOpen(3); // old 0
        if (GPIO_Handle < 0)
        {
            Debug("lgGpiochipOpen Failed: %d\n", GPIO_Handle); // old "gpiochip0 Export Failed\n"
            return -1;
        }
    }
    SPI_Handle = lgSpiOpen(0, 0, 100000, 0); // old 0,0,10000000,0
    if (SPI_Handle < 0)
    {
        Debug("lgSpiOpen Failed: %d\n", SPI_Handle); // old "SPI Export Failed\n"
        return -1;
    }
    IIC_Handle = lgI2cOpen(3, IIC_Address, 0); // old 1,IIC_Address,0
    if (IIC_Handle < 0)
    {
        Debug("lgI2cOpen Failed: %d\n", IIC_Handle); // old "I2C Export Failed\n"
        return -1;
    }
    ...
    DEV_HARDWARE_I2C_begin("/dev/i2c-3"); // old "/dev/i2c-1"
    DEV_HARDWARE_I2C_setSlaveAddress(IIC_Address);

    DEV_HARDWARE_SPI_begin("/dev/spidev0.0");
    DEV_HARDWARE_SPI_setSpeed(100000); // old 10000000
#endif
    return 0;
}
